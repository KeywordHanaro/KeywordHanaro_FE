...
const categories = [
  { name: '송금', description:'(송금, 입금, 출금, 예적금 등)',  path: '/ticket/detail?task=송금' },
  { name: '개인 대출', path: '/ticket/detail?task=개인 대출' },
  { name: '기업 대출', path: '/ticket/detail?task=기업 대출' }
];
        ...
        >
          <div className='flex flex-row gap-[8px] items-center'>
            ...
            <h1 className='font-bold text-[17px]'>{category.name}</h1>
						<small className='text-[12px] font-bold text-descriptionGray'>{category.description}</small>
          </div>
        </Card>
      ))}
    </div>
